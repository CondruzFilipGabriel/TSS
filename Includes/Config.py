...
def _cfg(section: str, key: str, default):
    section_data = USER_CONFIG.get(section, {}) if isinstance(USER_CONFIG, dict) else {}
...
class Timeouts:
    ...
    timeout_sec: int = field(default_factory=lambda: _cfg("timeouts", "timeout_sec", 180))
    timeout_sec_mutmut: int = field(default_factory=lambda: _cfg("timeouts", "timeout_sec_mutmut", 600))
    timeout_categorie_ai_sec: int = field(default_factory=lambda: _cfg("timeouts", "timeout_categorie_ai_sec", 1800))
    max_corectie_attempts: int = field(default_factory=lambda: _cfg("timeouts", "max_corectie_attempts", 1))
    max_empty_answers_consecutive: int = field(default_factory=lambda: _cfg("timeouts", "max_empty_answers_consecutive", 2))
...
class FilesConfig:
    ...
    ollama_prompts_log_name: str = "ollama_prompts.log"
    ollama_responses_log_name: str = "ollama_responses.log"
...
class GenerationLimits:
    """
    Grupeaza limitele folosite in buclele de generare.

    Aceste valori controleaza cat de repede trece framework-ul la urmatorul
    subtip atunci cand modelul nu mai aduce progres. Sunt tinute in config
    pentru a evita modificarea codului la fiecare reglaj experimental.
    """

    max_existing_subtype_attempts_without_progress: int = field(default_factory=lambda: _cfg("generation_limits", "max_existing_subtype_attempts_without_progress", 3))
    max_discovery_attempts_without_progress: int = field(default_factory=lambda: _cfg("generation_limits", "max_discovery_attempts_without_progress", 3))
    max_failed_attempts_kept_per_scope: int = field(default_factory=lambda: _cfg("generation_limits", "max_failed_attempts_kept_per_scope", 3))
...
class OllamaConfig:
    ...
    model: str = field(default_factory=lambda: _cfg("ollama", "model", "qwen2.5-coder:7b"))
    host: str = field(default_factory=lambda: _cfg("ollama", "host", "127.0.0.1"))
    port: int = field(default_factory=lambda: _cfg("ollama", "port", 11434))
    generate_endpoint: str = field(default_factory=lambda: _cfg("ollama", "generate_endpoint", "/api/generate"))
    tags_endpoint: str = field(default_factory=lambda: _cfg("ollama", "tags_endpoint", "/api/tags"))
    keep_alive: str = field(default_factory=lambda: _cfg("ollama", "keep_alive", "5m"))
    temperature: float = field(default_factory=lambda: _cfg("ollama", "temperature", 0.1))
    api_ready_timeout_sec: int = field(default_factory=lambda: _cfg("ollama", "api_ready_timeout_sec", 3))
    start_wait_timeout_sec: int = field(default_factory=lambda: _cfg("ollama", "start_wait_timeout_sec", 20))
    start_poll_interval_sec: float = field(default_factory=lambda: _cfg("ollama", "start_poll_interval_sec", 0.5))
...
class ProjectPaths:
    ...
    def ollama_log_file(self) -> Path:
        """Fisierul JSONL pentru prompt + raspuns Ollama, in aceeasi intrare."""
        return self.debug_dir / self.files.ollama_log_name

    @property
    def ollama_prompts_log_file(self) -> Path:
        """Fisierul JSONL pentru prompturile complete trimise catre Ollama."""
        return self.debug_dir / self.files.ollama_prompts_log_name

    @property
    def ollama_responses_log_file(self) -> Path:
        """Fisierul JSONL pentru raspunsurile brute primite de la Ollama."""
        return self.debug_dir / self.files.ollama_responses_log_name
...
class TerminalConfig:
    """Controleaza cat de mult se afiseaza in terminal."""

    show_ollama_prompt: bool = field(default_factory=lambda: _cfg("terminal", "show_ollama_prompt", False))
    show_ollama_response: bool = field(default_factory=lambda: _cfg("terminal", "show_ollama_response", False))
    show_ai_technical_messages: bool = field(default_factory=lambda: _cfg("terminal", "show_ai_technical_messages", False))


@dataclass(frozen=True)
class LoggingConfig:
    """Controleaza logurile persistente."""

    save_ollama_chat: bool = field(default_factory=lambda: _cfg("logging", "save_ollama_chat", True))
    save_ollama_prompts: bool = field(default_factory=lambda: _cfg("logging", "save_ollama_prompts", True))
    save_ollama_responses: bool = field(default_factory=lambda: _cfg("logging", "save_ollama_responses", True))
